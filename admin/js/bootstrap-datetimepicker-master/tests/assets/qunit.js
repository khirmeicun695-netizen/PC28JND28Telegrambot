...
QUnit.load = function() {
	...
	var configHtml = '', len = config.config.length;
	for ( var i = 0, val; i < len; i++ ) {
		val = config.config[i];
		config[val] = QUnit.urlParams[val];
		configHtml += '<label><input name="' + val + '" type="checkbox"' + ( config[val] ? ' checked="checked"' : '' ) + '>' + val + '</label>';
	}
	...
	if ( banner ) {
		banner.innerHTML = '<a href="' + QUnit.url({ filter: undefined }) + '"> ' + banner.innerHTML + '</a> ' + configHtml;
